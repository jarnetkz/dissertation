...
dt_save = 0.05           # save solution (snapshots)
tol = 1e-16
...
# PRESSURE PARAMETERS
# K1_star = params["_k"] /params["_mu"]                     
# K2_star = K1_star * 10
# U0 = params["_u_x"]     # velocity scale
# A_star = params["_u_x"] * params["_L"] / K1_star
...
def solve_pde(T, no_cells, dt, target_step, dir_path, logfile, params):
    ...
    while t < T + 1e-12:
        ...
        print(f"Step: {cnt}/{T*1/dt}| time step t={t}| Next Save={next_save} | mesh={no_cells} | velo={params['U0']:.3e} | omega={params['omega']}")
...
def plot_guassian_sol(meshsize, result, y_vals, physical_params):
    ...
    keys = list(result["c_line_by_t"]["line_x_x0"].keys()) # e.g.,t=0,0.5,1
