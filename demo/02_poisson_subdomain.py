...
# WHY inclusive operator : use >= and <= in both region 
    # points near this line x[0] = x_interface - tol are included in both subdomains
    # we don't use ">" , "<" because points exactly on the line x[0] = x_interface - tol would belong to neither subdomains
# WHY tolerance
...
log_path = "02_poisson_subdomain/subdomain_log.txt"
logfile = open(log_path, "w")


vtkfile = File('02_poisson_subdomain/solution.pvd')
...
while t < T + tol:
    ...
    p_exact = Expression(
        ...
        K1=float(K1),
        K2=float(K2),
        D=x_interface/float(K2) + (L-x_interface)/float(K1)
    )
