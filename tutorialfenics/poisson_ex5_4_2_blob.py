...
def solve_for_mesh(no_cells, logfile, target_step):
    ...
    while t < T + 1e-12:
        ...
        cnt += 1
        print(f"Step: {cnt}/{T*1/dt}, @time step t: {t}")
...
def compute_error_norms(final_result, norm_target_step, t_referrence):
    '''
    Find relative norm error 
    [norm (at different mesh size) - norm (finest mesh size)] / norm (finest mesh size)

    '''
    error_norms = {}
    for mesh_size in mesh_sizes:
        abs_error_norm = abs(final_result[mesh_size]["l2norm"][norm_target_step]           # norm of different mesh sizes    
                    - final_result[t_referrence]["l2norm"][norm_target_step]        # norm of the finest mesh size
                    )
        error_norms[mesh_size] = abs_error_norm
    return error_norms
